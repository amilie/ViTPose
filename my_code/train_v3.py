...
from model import RadarPoseResTransformer, WingLoss
...
def main() -> None:
    root_dir = "../P1"
    batch_size = 16
    learning_rate = 3e-4     # slightly lower than v2: deeper transformer needs careful LR
    num_epochs = 80          # more room: 4-layer transformer takes longer to converge
    train_ratio = 0.8
    random_seed = 42
    patience = 15            # more patience for the deeper model
    grad_clip = 0.5          # tighter: deeper attention can spike gradients
    weight_decay = 5e-5      # lighter: spatial token pooling is already regularised
    ...
    # WingLoss: targets small-error accuracy which matters most for joint localisation
    criterion = WingLoss(w=10.0, eps=2.0)

    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay)

    # Linear warmup for 8 epochs, then cosine decay to 1e-6
    # Warmup is longer here: 4-layer transformer + pos embeddings need gentle start
    def lr_lambda(epoch):
        warmup = 8
        if epoch < warmup:
    ...
    epochs_no_improve = 0
    output_dir = Path("checkpoints_v3")
    ...
    for epoch in range(num_epochs):
        ...
        fig.suptitle("Training Curves (v3)", fontsize=13, fontweight="bold")
